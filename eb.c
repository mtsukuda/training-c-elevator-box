...
//  引  数：int    呼び出された階
//       ：int    行きたい方向：1:⬆️ 2:⬇️
//       ：elvbox    現在のエレベーター情報が入っている配列
//       ：int    エレベーターカゴの最大値
//  戻り値：一番近いエレベーターカゴ番号（0スタート）
//------------------------------------------------
int findElevatorBox (int myFloor, int myDirection, elvbox boxes[], int elvMax)
{
    // 課題の条件は以下の通り
    // ①一番近いエレベーター
    // ②方向が同じ or 停止中
...
int main()
{
    ...
    for (;;) {
        ...
        /* 入力された方向を myDirection 変数にセット */
        myDirection = atoi(command);
        break;
    }
    ...
    // 一番近いエレベーターカゴを探して返す
    printf("\n==> 一番近いエレベーターカゴ🛗 : %d号基\n", findElevatorBox(myFloor, myDirection, boxes, 3) + 1);
